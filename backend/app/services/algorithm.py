...
def select_items_for_budget(
    ...
    # Если бюджета на это место не хватает даже на самый дешёвый пигмент —
    # подарок честно остаётся без пигментов (только обязательные расходники),
    # а не «занимает» один и тот же пигмент на всех местах в обход бюджета
    # (это раньше стирало разницу между 1/2/3 местом и срывало бюджет вверх).
